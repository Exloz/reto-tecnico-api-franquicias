...
public class DeleteBranchProductUseCase {
    ...
    public Mono<Void> execute(UUID franchiseId, UUID branchId, UUID productId, long expectedVersion) {
        return Mono.fromCallable(() -> DomainRules.validateExpectedVersion(expectedVersion))
                .flatMap(version -> UseCaseSupport.requireResource(
                ...
                .flatMap(product -> UseCaseSupport.requireVersion(
                        product, product.getVersion(), version, "Product", productId))
